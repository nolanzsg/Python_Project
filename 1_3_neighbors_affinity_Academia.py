#Filename: 1_3_neighbors_affinity_Academia.py
...
data = open('C:\users\zhusuoge\desktop\Academia2Anonymized.csv')
#data.readline()
...
def neighbors_affinity(m, n):
    ...
    for x in G.neighbors(m):
        for y in G.neighbors(n):
            if G.has_edge(x, y):
                count = count + 1
    return float(count)/float((math.log10(len(G.neighbors(m))+2))*(math.log10(len(G.neighbors(n))+2)))
...
#training set
f =file('1_3_neighbors_affinity_academia.csv','w')
writer = csv.writer(f)
writer.writerow(['node_out', 'node_in',
                 'edge_linked(m, n)', 'common_friends(m, n)',
                 'total_friends(m, n)', 'jaccard_coefficient(m, n)',
                 'preferental_attachment_score(m, n)', 'neighbors_affinity(m, n)'
                 ])

count_number = 0
while count_number < 5000:
    count_number = count_number +1
